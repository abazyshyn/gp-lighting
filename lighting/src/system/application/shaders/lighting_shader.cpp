...
namespace GP
{
    ...
    bool CLightingShader::InitShader(ID3D11Device *device, HWND hWnd, const std::filesystem::path &vsFilename, const std::filesystem::path &psFilename)
    {
        ...
        if (FAILED(D3DCompileFromFile(vsFilename.c_str(), nullptr, nullptr, "LightingVertexShader", "vs_5_0",
                                      D3D10_SHADER_ENABLE_STRICTNESS, 0, &vertexShaderBuffer, &errorMessageBuffer)))
        {
            if (errorMessageBuffer)
            {
                OutputShaderErrorMessage(errorMessageBuffer, hWnd, vsFilename);
            }
            else
            {
        ...
        if (FAILED(D3DCompileFromFile(psFilename.c_str(), nullptr, nullptr, "LightingPixelShader", "ps_5_0",
                                      D3D10_SHADER_ENABLE_STRICTNESS, 0, &pixelShaderBuffer, &errorMessageBuffer)))
        {
            if (errorMessageBuffer)
            {
                OutputShaderErrorMessage(errorMessageBuffer, hWnd, vsFilename);
            ...
            {
                MessageBox(hWnd, std::filesystem::absolute(vsFilename).c_str(), L"Missing LightingPixelShader File", MB_OK);
            }
        ...
        if (FAILED(device->CreateInputLayout(polygonLayout.data(), static_cast<uint32_t>(polygonLayout.size()),
        ...
        lightBufferDescription.ByteWidth = static_cast<uint32_t>(sizeof(LightBuffer_s));
        ...
        if (FAILED(device->CreateBuffer(&lightBufferDescription, nullptr, &m_lightBuffer)))
        {
            MessageBox(hWnd, L"Could not create ID3D11Buffer m_lightBuffer", L"Error", MB_OK);
            return false;
        }
