...
namespace GP
{
    ...
    bool CDirect3D::Init(int32_t windowWidth, int32_t windowHeight, bool vsync, HWND hWnd, bool fullscreen, float nearPlane, float farPlane)
    {
        ...
        std::vector<DXGI_MODE_DESC> displayModes(static_cast<size_t>(countModes));
        ...
        size_t stringLength{};
        constexpr size_t SZ_BYTES = 128;
        if (wcstombs_s(&stringLength, m_videoCardDescription, SZ_BYTES, adapterDescription.Description, SZ_BYTES))
        {
        ...
        swapChainDescription.BufferDesc.Width = static_cast<uint32_t>(windowWidth);
        swapChainDescription.BufferDesc.Height = static_cast<uint32_t>(windowHeight);
        ...
        m_perspectiveMatrix = XMMatrixPerspectiveFovLH(fieldOfView, windowAspectRation, nearPlane, farPlane);
    ...
    {
        constexpr size_t SZ_BYTES = 128;
        strcpy_s(cardName, SZ_BYTES, m_videoCardDescription);
